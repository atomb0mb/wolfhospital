...
stmt.executeUpdate("DROP TABLE Hospital IF EXISTS");
stmt.executeUpdate("DROP TABLE Specializations IF EXISTS");
stmt.executeUpdate("DROP TABLE Beds IF EXISTS");
stmt.executeUpdate("DROP TABLE Staff IF EXISTS");
stmt.executeUpdate("DROP TABLE Patient IF EXISTS");
stmt.executeUpdate("DROP TABLE CheckIn IF EXISTS");
stmt.executeUpdate("DROP TABLE MedicalRecords IF EXISTS");
stmt.executeUpdate("DROP TABLE BillingAccounts IF EXISTS");

// Create the table for Admin

stmt.executeUpdate("create table Admin(aID integer NOT NULL UNIQUE, aName varchar(50) NOT NULL, PRIMARY KEY(aID)");
...
FOREIGN KEY (sID) REFERENCES Specializations(sID), FOREIGN KEY (hID) REFERENCES Hospital(hID), FOREIGN KEY (pID) REFERENCES Patient(pID)");
...
// Specializations
stmt.executeUpdate("insert into Specializations(sID, hID, sName, costPerDay) VALUES (1, 11101, 'pediatrics', 10)");
stmt.executeUpdate("insert into Specializations(sID, hID, sName, costPerDay) VALUES (2, 11101, 'neurology', 15)");

// hospital #2
stmt.executeUpdate("insert into Hospital(hID, aID, hAddress, hPhone, capacity) VALUES (222, 22201, '222 St NC, 222', '202', 200)");
// Specializations
stmt.executeUpdate("insert into Specializations(sID, hID, sName, costPerDay) VALUES (3, 22201, 'cardiology', 20)");
stmt.executeUpdate("insert into Specializations(sID, hID, sName, costPerDay) VALUES (4, 22201, 'oncology', 25)");
